...
function provision(user, pw, options, cb){
  ...
  mongo.connect(mongoUrl, function(e, db){
    if(e) return cb(e);
    if(db.getUsers().length === 0) { //this makes sure the database wasn't already created
        db.addUser(config.userName, config.userPassword, {role: options.userRole}, function(err, res){
            if(err) return cb(err); //may need to check for existing db or user (error code 11000? for user already exists)
            return cb(null, config);
        });
    }
    else{
        return new Error('A database with this name already exists');
    }
  });
}
