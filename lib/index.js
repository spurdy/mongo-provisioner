...
function buildConnectionString(user, pw, options){
    if(user && pw){ //connect with a username and password
        ...
        var format = "mongodb://%s:%s/%s?authSource=%s";
...
function provision(user, pw, options, cb){
  ...
  mongo.connect(mongoUrl, function(e, db){
    ...
    db.command({usersInfo: 1}, function(err, result){ //make sure the database wasn't already created
        if (result.users.length===0){ // no users are in the database, can assume it is new
            db.addUser(config.userName, config.userPassword, {role: options.userRole}, function(err, res){
                if(err) return cb(err);
                    return cb(null, config);
            });
        }
        else if (options.allowExistingDb) {
            db.addUser(config.userName, config.userPassword, {role: options.userRole}, function(err, res){
                if(err) return cb(err);
                return cb(null, config);
            });
        }
        else return cb(new Error('A database with this name already exists and the \'allowExistingDb\' option is false'));
    })
  });
}
